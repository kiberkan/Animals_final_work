...
public class Validator {
    ...
    private boolean isValidName (String name){
        for (int i = 0; i < name.length(); i++) {
            if (! Character.UnicodeBlock.of(name.charAt(i)).equals(Character.UnicodeBlock.CYRILLIC)) {
                throw new UncorrectDataException(String.format("некорректно введено имя, допустима только кириллица"));
            }
        }
        return true;
    }
    ...
    private boolean isValidDate (String birthday)  {
        ...
        try {
            ...
            throw new UncorrectDataException("неверный формат даты");
        }
